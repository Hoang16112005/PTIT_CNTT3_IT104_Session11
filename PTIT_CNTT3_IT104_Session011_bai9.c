...
// xây dựng thêm hàm đảo ngược danh sách liên kết đã được tạo
...
Node* daoNguocDanhSach(Node* head) {
    ...
    // Đảo ngược các con trỏ next và prev
    while (current != NULL) {
        temp = current->prev;
    ...
    // Cập nhật head về phần tử cuối cùng
    if (temp != NULL) {
        head = temp->prev;
    }
...
int main() {
    ...
    head = daoNguocDanhSach(head);
    printf("Danh sách liên kết đôi sau khi đảo ngược: ");
    duyetToanBoDanhSach(head);
